...
{
	if (R_Bit(DMA->DMA_INT_STA_REG, ndma_index << 1))
	{
		S_Bit(DMA->DMA_INT_STA_REG, ndma_index << 1);
		return 1;
	}
	return 0;
}
...
{
	if (R_Bit(DMA->DMA_INT_STA_REG, (ndma_index << 1) + 1))
	{
		S_Bit(DMA->DMA_INT_STA_REG, (ndma_index << 1) + 1);
		return 1;
	}
	return 0;
}
