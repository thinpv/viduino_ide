...
volatile static __u32 de_scal_trd_fp_en = 0;
static __u32 de_scal_trd_itl_en = 0;
volatile static __u32 de_scal_ch0r_offset;
volatile static __u32 de_scal_ch1r_offset;
volatile static __u32 de_scal_ch2r_offset;
																 ...
																 __scal_out_size_t *out_size, __scal_out_type_t *out_type)
{
	...
	w_shift = (in_type->fmt == DE_SCAL_INYUV411) ? 2 : ((in_type->fmt == DE_SCAL_INYUV420) || (in_type->fmt == DE_SCAL_INYUV422)) ? 1
																																																																: 0;
															 ...
															 __scal_out_size_t *out_size, __scal_out_type_t *out_type, __u8 smth_mode)
{
	...
	ch0v_fir_coef_ofst = (int_part == 0) ? zoom0_size : (int_part == 1) ? zoom0_size + float_part
	...
	ch0h_fir_coef_ofst = (int_part == 0) ? zoom0_size : (int_part == 1) ? zoom0_size + float_part
																									: (int_part == 2)		? zoom0_size + zoom1_size + (float_part >> 1)
																									: (int_part == 3)		? zoom0_size + zoom1_size + zoom2_size
																									: (int_part == 4)		? zoom0_size + zoom1_size + zoom2_size + zoom3_size
																																			: zoom0_size + zoom1_size + zoom2_size + zoom3_size + zoom4_size;
	...
	ch1v_fir_coef_ofst = (int_part == 0) ? zoom0_size : (int_part == 1) ? zoom0_size + float_part
																									: (int_part == 2)		? zoom0_size + zoom1_size + (float_part >> 1)
																									: (int_part == 3)		? zoom0_size + zoom1_size + zoom2_size
																									: (int_part == 4)		? zoom0_size + zoom1_size + zoom2_size + zoom3_size
																																			: zoom0_size + zoom1_size + zoom2_size + zoom3_size + zoom4_size;
	...
	ch1h_fir_coef_ofst = (int_part == 0) ? zoom0_size : (int_part == 1) ? zoom0_size + float_part
																									: (int_part == 2)		? zoom0_size + zoom1_size + (float_part >> 1)
																									: (int_part == 3)		? zoom0_size + zoom1_size + zoom2_size
																									: (int_part == 4)		? zoom0_size + zoom1_size + zoom2_size + zoom3_size
																																			: zoom0_size + zoom1_size + zoom2_size + zoom3_size + zoom4_size;
...
void Defe_Init(void)
{
	Open_Dev_Clock(DEFE_CLOCK);									//Open the clock
	DE_SCAL_Set_Reg_Base(0, DEFE_Base_Address); //Set base address
	DE_SCAL_Reset(0);
	DE_SCAL_Enable(0);									//使能
	C_BIT(DEFE_Base_Address + 0X0, 31); //set to 0 when using DEFE

	//开启中断
	DE_SCAL_EnableINT(0, Wb_end | Line); //回写结束中断
	SET_TRIG_LINE(271);									 //线中断
																			 // 	IRQ_Init(IRQ_LEVEL_1,IRQ_DE_FE,IRQ_DEFE_ISR ,3);
																			 //   sysSetLocalInterrupt(ENABLE_IRQ);//开IRQ中断
	irq_register(IRQ_LEVEL_1, F1C100S_IRQ_DEFE, IRQ_DEFE_ISR, 3);
}
/*********************************************************************************************
DEFE配置
...
void Defe_Config_argb8888_to_argb8888(int inx, int iny, int outx, int outy, int argb8888)
{
	...
	//-------------------------Output
	volatile __scal_buf_addr_t backaddr;
	...
	src_buf_addr.ch0_addr = (u32)argb8888;
	...
	src_type.byte_seq = D3D2D1D0;
	src_type.mod = DE_SCAL_INTER_LEAVED;
	src_type.fmt = DE_SCAL_INRGB888;
	src_type.ps = 0;
	DE_SCAL_Config_Src(0, &src_buf_addr, &src_size, &src_type, 0, 0); //Configuration input

	//-------------------------------------------输出
#ifdef TV_OUT
	out_scan.field = 1; //0:frame scan; 1:field scan //Set to field scan when TV output
#else
	out_scan.field = 0; //0:frame scan; 1:field scan //Set to frame scan when TV output
#endif
	...
	DE_SCAL_Set_CSC_Coef(0, csc_bt601, csc_bt601, csc_RGB, csc_RGB, 0, 0); //Color conversion factor
...
// {
// 	Defe_Config_argb8888_to_argb8888(NES_DW,NES_DH,XSIZE_PHYS,YSIZE_PHYS,src_addr);
// }
/*********************************************************************************************
DEFE switch cache
*********************************************************************************************/
