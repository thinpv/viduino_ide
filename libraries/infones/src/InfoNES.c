...
/* RAM */
BYTE NES_RAM[ RAM_SIZE ];


/* SRAM */
BYTE NES_SRAM[ SRAM_SIZE ];
...
DWORD DoubleFrame[ 2 ][ NES_DISP_WIDTH * NES_DISP_HEIGHT ];
DWORD *WorkFrame;
DWORD WorkFrameIdx;
#else
...
#else
#ifdef DISP_USE_ARGB_32
DWORD WorkFrame[ NES_DISP_WIDTH * NES_DISP_HEIGHT ];
#else
...
/*                                                                   */
/*===================================================================*/
int InfoNES_Load( unsigned char *dat )
{
/*
 *  Load a cassette
 *
 ...
 */
  ...
  {
    // Non support mapper
    printf( "Mapper #%d is unsupported.\n", MapperNo );
    return -1;
  }
 ...
 */
  ...
  if ( !( PPU_R1 & R1_SHOW_SCR ) )
  {
    ...
    InfoNES_MemorySet( pPoint, 0, NES_DISP_WIDTH * sizeof(pPoint[0]) );
  ...
  else
  {
    ...
    if ( !( PPU_R1 & R1_CLIP_BG ) )
    {
      ...
      InfoNES_MemorySet( pPointTop, 0, 8 *sizeof(pPointTop[0]) );
    }
       ...
       ( SCAN_ON_SCREEN_START > PPU_Scanline || PPU_Scanline > SCAN_BOTTOM_OFF_SCREEN_START ) )
    {
      ...
      pPointTop = &WorkFrame[ PPU_Scanline * NES_DISP_WIDTH ];
      InfoNES_MemorySet( pPointTop, 0, NES_DISP_WIDTH *sizeof(pPointTop[0]));
    }  
  }
