...
#include "FT5xx6.h"
  ...
  bool preIntStatus = LOW;
  bool FT5xx6::touched(){
//    if(digitalRead(_ctpInt) == LOW) {/
    if(gpio_get_value(GPIOA, 1) == LOW) {
      if(preIntStatus == HIGH) {
        preIntStatus = LOW;
        return true;
      }
    } else {
//      if(pre/IntStatus == LOW) {
        preIntStatus = HIGH;
//        return true;/
//      }/
    }
...
//    }
  ...
  void FT5xx6::init(bool serial_output_enabled) {
    ...
    gpio_set_dir(GPIOA, 1, GPIO_DIRECTION_INPUT);
//    gpio_set_pull(GPIOE, 5, GPIO_PULL_DOWN/);
