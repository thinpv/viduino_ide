...
int UnpackFrameHeader(MP3DecInfo *mp3DecInfo, unsigned char *buf)
{
	...
	if(get_fw()==0)
	{
		...
		if (!mp3DecInfo || !mp3DecInfo->FrameHeaderPS || (buf[0] & SYNCWORDH) != SYNCWORDH || (buf[1] & SYNCWORDL) != SYNCWORDL)return -1;		
	}
	if(get_fw()==1)//Add
	{
		/* validate pointers and sync word */
		if (!mp3DecInfo || !mp3DecInfo->FrameHeaderPS || (buf[0] & SYNCWORDH_x) != SYNCWORDH_x || (buf[1] & SYNCWORDL_x) != SYNCWORDL_x)return -1;		
	}
