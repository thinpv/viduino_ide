...
void Mapper001_Reset()
{
  ...
  size_in_K = (RomHeader->num_16k_rom_banks << 1) * 8; //16kB ROM����Ŀ
  ...
  MAP1->Map1_256K_base = 0; // use first 256Kʹ�õ�һ��256 k
  MAP1->Map1_swap = 0;

  if (MAP1->Map1_Size == Map1_SMALL)
  {
    // set two high pages to last two banks�����ߵ�ҳ������Ϊ���
...
void Mapper001_Write(uint8 byData, uint16 wAddr) //VROM_1K_SIZE
{
  ...
  if (byData & 0x01)
    MAP1->Map1_Latch |= (1 << MAP1->Map1_Cnt);
  MAP1->Map1_Cnt++;
  if (MAP1->Map1_Cnt < 5)
    return;
  ...
  case 0:
  {
    ...
    if (MAP1->Map1_Regs[0] & 0x02)
    {
      ...
      {
        PPU_set_mirroring(0, 0, 1, 1); //ˮƽ����InfoNES_Mirroring( 0 );   //0011
      }
      else
      {
        PPU_set_mirroring(0, 1, 0, 1); //��ֱ����InfoNES_Mirroring( 1 );   //0101
      }
    }
    else
    {
