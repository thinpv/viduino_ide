...
//NES显示缓存
__attribute__((aligned(1024))) unsigned int NES_BUFF[2][NES_DH][NES_DW];
...
void scanline_draw(int LineNo)
{
	...
	for (i = 0; i < NES_DW; i++)
	{
		NES_BUFF[buff_inx][LineNo][i] = NES_Palette_argb8888[ppu->dummy_buffer[i + 8]];
		// LCDbuff[LV_HOR_RES_MAX*LineNo + i] = NES_Palette_argb8888[ppu->dummy_buffer[i+8]];
	}

	if (LineNo >= 239) //一帧结束-切换下一个缓存
	{
		defe_inx = buff_inx;
		//切换下一个nes缓存
		buff_inx++;
		...
		// if (defe_inx >= 2)
		// 	defe_inx = 0;
		// Defe_conversion_buff((u32)(LCDbuff));//thinpv
		// Defe_conversion_buff((u32)(NES_BUFF[defe_inx]));
		//开启defe
		if (video_run_callback)
			video_run_callback(0, 0, NES_DW, NES_DH, (unsigned int *)NES_BUFF[defe_inx]);
		// fb_area_present(0, 0, NES_DW, NES_DH, (unsigned short *)NES_BUFF[defe_inx]);
		// buff_inx_n++;
		// if (buff_inx_n == 3)
		// {
		// Enable_Layer_Video(); //thinpv
		// #define F1C100S_DEBE_BASE (0x01e60000)
		// 			struct f1c100s_debe_reg_t *debe = (struct f1c100s_debe_reg_t *)F1C100S_DEBE_BASE;
		// 			S_BIT((virtual_addr_t)&debe->layer0_attr0_ctrl, 1); //使能视频功能
		// }
	}
}
...
// 	int i = 0;
// 	for (i = 0; i < 64; i++)
// 		NES_Palette_argb8888[i] = ((u64_t)0xff << 24) |														 //A
